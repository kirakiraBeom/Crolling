...
import time
from coupanq.ExcelConverter import EXCELConverter
...
def show_keywords():
    def add_keyword():
        ...
        if keyword and new_name:
            ...
            messagebox.showwarning(
                "입력 필요", "키워드와 상품명을 모두 입력하세요.", parent=keyword_window
            )
        ...
        if selected:
            ...
            if keyword in keyword_mapping:
                ...
                if selected_index < listbox.size():  # 삭제된 항목이 마지막이 아닐 때
                    ...
                    listbox.size() > 0
                ):  # 삭제된 항목이 마지막이고 리스트에 다른 항목이 있을 때
            ...
            messagebox.showwarning(
                "선택 필요", "삭제할 키워드를 선택하세요.", parent=keyword_window
            )
    ...
    def show_overview():
        ...
        text_box = tk.Text(
            overview_window, wrap="word", bg="#ffffff", fg="#000000", font=("Arial", 10)
        )
        ...
        def search_text(event=None):
            ...
            tk.Label(search_window, text="찾을 단어:", bg="#f0f8ff", fg="#000000").pack(
                pady=5
            )
            ...
            def perform_search():
                ...
                if not search_query:
                    messagebox.showwarning(
                        "입력 필요", "검색어를 입력하세요.", parent=search_window
                    )
                    return
    ...
    def search_keyword():
        ...
        tk.Label(search_window, text="찾을 키워드:", bg="#f0f8ff", fg="#000000").pack(
            pady=5
        )
        search_entry = tk.Entry(search_window)
        search_entry.pack(pady=5)
        ...
        def perform_search(event=None):
            ...
            if not search_text:
                messagebox.showwarning(
                    "입력 필요", "검색할 키워드를 입력하세요.", parent=search_window
                )
                return
            ...
            if not found:
                messagebox.showinfo(
                    "검색 결과",
                    f"'{search_text}'에 해당하는 키워드를 찾을 수 없습니다.",
    ...
    listbox = tk.Listbox(
        keyword_window, width=50, height=15, bg="#ffffff", fg="#000000"
    )
    ...
    add_button = tk.Button(
        button_frame, text="추가", command=add_keyword, bg="#add8e6", fg="#000000"
    )
    add_button.grid(row=0, column=0, padx=5)

    delete_button = tk.Button(
        button_frame, text="삭제", command=delete_keyword, bg="#add8e6", fg="#000000"
    )
    ...
    try:
        ...
        def update_product_name(row):
            ...
            # 상품명이나 옵션정보에 "1+1", "1set+1set"이 있으면 상품명 앞에 추가
                ...
                "1+1" in combined_text or "1set+1set" in combined_text
            ) and "미니 브러쉬" not in combined_text:
                product_name = f"{product_name} 1+1"

            if (
                "1+2" in combined_text
                and "코일" not in combined_text
                and "엣지형 변경" not in combined_text
            ):
        ...
        save_path = os.path.join(
            directory, f"res_{base_name}.xlsx"
        )  # 경로와 파일명 결합
        counter = 1
        while os.path.exists(save_path):
            save_path = os.path.join(
                directory, f"res_{base_name}_{counter}.xlsx"
            )  # 중복 시 번호 추가
            counter += 1
...
def decrypt_excel(file_path, password):
    try:
        ...
        messagebox.showerror(
            "오류", f"암호화된 파일을 해제하는 중 오류가 발생했습니다: {str(e)}"
        )
        return None
...
def setup_gui():
    ...
    # ExcelConverter.py 실행
    ExcelConverter_button = tk.Button(
        root,
        text="쿠팡 파일 선택",
        command=EXCELConverter.run,
        bg="#ffb6c1",
        fg="#000000",
        font=("Arial", 12, "bold"),
    )
    ExcelConverter_button.pack(pady=10)
