...
keywords_to_remove = [
    r"선택\d",r"사이즈 선택:",r"향 선택:", r"모델명:",r"수량:",r":\d+\.",r"/\d+\.","차량용", "벤오토", "센트럴리빙", "컵홀드", "종류","와이퍼", r"제품:\d+\.?", r"\(\+\d+원\)", r"\(\-\d+원\)", r"\d+-\d+\.",r"\(\d+원\)",
    r"\[한정수량\]", r"선택:", r"선택 :",r"선택\d+:",r"선택\.\d+", r"색상:",r"컬러:" ,r"\/\-\d+원\/",r"\/\+\d+원\/",r"\/\d+원\/",
    r"\/",r"\:", r"\,", r"옵션=",r"선택=",r"\[국내배송\]",r"선택\s+\d", r"\(\+\d+\)","mm",r"색상=",r"사이즈=","목베개 목받침 헤드레스트 메모리폼","자동차",
    r"브랜드=", r"모델명=",r"추가:",r"추가 :","코란도","선택","차종","="
]

keyword_replacements = {
    "^": " ", "자동차 초미세모  세차":" ", "자동차 초미세모 세차":" ", "운전석 조수석 세트": " ",r"[1+1 원쁠]":" ","자동차 청소 디테일링 초미세모":" ",
    r"(1대분은 ★2개★ 구매바랍니다.)":"", r"=(1103~1506)":" ",r"(1103~1506)":" ", r"(배송매세지에남겨주세요)":" ",r"(배송메세지에남겨주세요)":" ",
    ...
    r"카니발 4세대 KA4 (20~)":" ",r"그랜드카니발 R (05~14)":" ",r"카니발 12 후방 (98~05)":" ",r"카니발 4세대 (20~)":" ",
    r"카니발 12 (98~05)":" ",r"뉴카니발 R (06~14)":" ",r"카니발 올뉴더뉴 (~20)":" ",r"카니발 KA4 PE (23~)":" ",
    ...
    r"LF 뉴라이즈（14 3~19）":" ", r"LF뉴라이즈(143~19)": " ", r"YF쏘나타(0909)":" ", r"NF쏘나타(049~098)":" ",r"LF 뉴라이즈（143~19）":" ",r"쏘나타 DN8":" ",
    ...
    r"그랜저 IG （16 11~）":" ", r"그랜저 IG (16 11~)":" ",

    r"올뉴모닝(1701~)": " ", r"올뉴모닝(1701~)": " ",r"모닝올뉴모닝(04~16)":" ",

    r"투싼NX4 (2009~)":" ",r"올뉴투싼 (1503~)":" ",r"투싼ix （0908~）":" ",r"투싼 （0404~0907）":" ",r"투싼ix (0908~)":" ",r"투싼 (0404~0907)":" ",

    r"SM3 뉴 (098~14년)":" ",r"SM3 (05~09년)":" ",r"SM5 뉴플래티넘노바": " ",r"SM5 임프레션(05~09)":" ",r"SM3 네오(14~19년)":" ",
    
    r"더뉴 QM6 (19년 ~ 현재)":" ", r"더뉴(19년~)":" ",
    ...
    r"K5 DL3 (20~)":" ",r"K5DL3 (20~)":" ", r"더뉴 K5 (104~156)":" ", r"더뉴K5 (104~156)":" ",r"올뉴 K5 (157~19)":" ",r"올뉴K5 (157~19)":" ",
    
    r"K7 더뉴올뉴 （12~19）":" ", r"K7 더프리스티지 （09~12）":" ", r"K7 프리미어 （196）":" ",
    r"K7 더뉴올뉴 (12~19)":" ", r"K7 더프리스티지 (09~12)":" ", r"K7 프리미어 (196)":" ",

    r"K8 GL3 PE (24~)":" ",r"K8 (21년~)":" ",
...
# 중복 처리를 할 키워드 리스트 추가
duplicate_keywords = [
    "수세미", "앤코코", "듀오세트","센트세이","먼지털이 미니 브러쉬","컵홀더","타이탄 청소기 헤파필터", "석고","아이온 휴대폰 거치대","워터리스 물기제거기"
]  # 원하는 키워드를 추가
...
def convert_shop_name(shop_name, product_name, option):
    ...
    if shop_name == "스마트스토어":
        if any(keyword in product_name or keyword in option for keyword in [
            ...
            "키체인", "키링", "2in1", "가죽", "와이퍼", "듄", "메모리폼", "석고방향제", "차량",
            "트렁크", "틈새가죽", "차종", "화이트골드", "틈새", "아이온", "일랑일랑", "송풍구",
            "딥 블랙", "엣지형", "블랙골드", "햇빛가리개", "정리광", "콘솔", "3열추가", "맥세이프",
            "도어가드", "사이즈", "포맥스", "컵홀더", "콘솔수납포켓", "타이탄 청소기", "물기제거기"
        ]):
            return "N-벤"
        
        elif any(keyword in product_name or keyword in option for keyword in [
            "센트럴리빙"
        ]):
            return "N"
    ...
    # "쿠팡"에 대한 처리
    if shop_name == "쿠팡":
        if any(keyword in first_file_data['쇼핑몰'].tolist() for keyword in ["현대이지웰", "신세계"]):
            # "현대이지웰" 또는 "신세계"가 있는 경우 항상 "쿠팡-벤"
            return "쿠팡-벤"
        else:
            # "현대이지웰" 또는 "신세계"가 없는 경우 조건에 따라 "쿠팡-벤" 또는 "쿠팡-케"
            if any(keyword in product_name or keyword in option for keyword in [
                "벤오토", "운전석", "조수석", "스트랩", "각인", "브러쉬", "카우하이드", r"추가:",
                "키체인", "키링", "2in1", "가죽", "와이퍼", "듄", "메모리폼", "석고방향제", "차량",
                "트렁크", "틈새가죽", "차종", "화이트골드", "틈새", "아이온", "일랑일랑", "송풍구",
...
def process_files(second_file_path):
    try:
        ...
        # 첫 번째 파일의 '옵션'에 '[듀오세트]',"석고방향제"가 있는 경우 삭제
        ...
        # 품목명에서 여러 개의 공백을 하나의 공백으로 축소
        df2['품목명'] = df2['품목명'].str.replace(r'\s+', ' ', regex=True).str.strip()
        ...
        # '숫자.'와 같은 숫자 제거 (단, '숫자.숫자' 형식은 제외)
        df2['품목명'] = df2['품목명'].apply(lambda x: re.sub(r"(?<!\d)\b(\d+\.)(?!\d)", "", x))
