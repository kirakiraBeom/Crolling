...
keywords_to_remove = [
    "차량용", "벤오토", "센트럴리빙", "컵홀드", "종류","와이퍼", r"제품:\d+\.?", r"\(\+\d+원\)", r"\(\-\d+원\)", r"\d+-\d+\.",
    r"\[한정수량\]", r"선택:", r"선택 :",r"선택\d+:", r"색상:",r"컬러:", r"\d+\.",r"블랙:", r"\/\-\d+원\/",r"\/\+\d+원\/",r"\/\d+원\/",
    r"\/",r"\:", r"\,", r"옵션=",r"선택=",r"\[국내배송\]",r"선택\s+\d", r"\(\+\d+\)","mm",r"색상=",r"사이즈=","목베개 목받침 헤드레스트 메모리폼","자동차",
    r"브랜드=", r"모델명=","차종",r"추가:",r"추가 :"
]
...
keyword_replacements = {
    ...
    r"올뉴모닝(1701~)": " ", r"LF뉴라이즈(143~19)": " ", r"스타리아(21년~)": " ",r"스타리아 （21년~）":" ",
    r"코란도C (11~18년)": " ",r"DM더스타일(05년~12년)": " ",r"팰리세이드 (19년~)": " ",r"토레스":" ",r"（그랜저）   더뉴그랜저IG （20~）":" ",
    r"아반떼 HD(0605)": " ", r"올뉴모닝(1701~)": " ", r"SM5 뉴플래티넘노바": " ", r"포르테쿱": " ",r"스포티지 NQ5（21년）":" ",
    r"스포티지 R（10 4~15 8）":" ", r"더뉴 레이":" ", r"셀토스더뉴셀토스":" ", r"봉고3":" ", r"SM3 뉴 (098~14년)":" ", r"올뉴마이티":" ",
    r"（그랜저）   그랜저 HG （11 01~）":" ",r"G330 G380（14~16년）":" ",r"더뉴 QM6 (19년 ~ 현재)":" ", r"（쏘나타）   LF 뉴라이즈（14 3~19）":" ",
    r"쏘렌토 MQ4（20년~）":" ",r"카니발 4세대 KA4 （20~）":" ",r"팰리세이드 （19년~）":" ",r"=(1103~1506)":" ",r"더뉴그랜드(18년~)":" ",r"NF쏘나타(049~098)":" ",
    r"베라크루즈 (11년~15년)":" ",r"카운티":" ",r"YF쏘나타(0909)":" ",r"SM3 (05~09년)":" ",r"올뉴말리부(1604~)":" ",r"투싼NX4 (2009~)":" ",r"올뉴투싼 (1503~)":" ",
    r"모닝올뉴모닝(04~16)":" ",r"그랜드카니발 R (05~14)":" ",
}
...
# 중복 처리를 할 키워드 리스트 추가
duplicate_keywords = [
    "수세미", "앤코코", "듀오세트","센트세이","먼지털이 미니 브러쉬","컵홀더","타이탄 청소기 헤파필터"
]  # 원하는 키워드를 추가
0
...
def clean_option(option):
    ...
    if "반사스티커" not in option:
        patterns = [
            r"x\s*(\d+)\s*개(?!입)",      # "x 1개" 뒤에 "입"이 없을 때만 별로 감싸기
            r"\((\d+)\s*개(?!입)\)",      # "(1개)" 뒤에 "입"이 없을 때만 별로 감싸기
            r"-\s*(\d+)\s*개(?!입)",         # "-1개" 뒤에 "입"이 없을 때만 별로 감싸기
            r"(\d+)\s*개(?!입)",          # "1개" 뒤에 "입"이 없을 때만 별로 감싸기
            r"\((\d+)\s*롤\)",            # "(1롤)" -> "★1롤★"
            r"(\d+)\s*롤",                # "1롤" -> "★1롤★"
            r"\(총\s*(\d+)\s*롤\)",       # "(총 1롤)" -> "★1롤★"
            r"\(총\s*(\d+)\s*개(?!입)\)", # "(총 1개)" 뒤에 "입"이 없을 때만 별로 감싸기
            r"총\s*(\d+)\s*개(?!입)",     # "총 1개" 뒤에 "입"이 없을 때만 별로 감싸기
        ]
...
def process_files(second_file_path):
    try:
        ...
        df2[['고객주문번호', '받는분전화번호', '받는분기타연락처', '받는분주소(전체 분할)', '배송메세지1']] = first_file_data[['사방넷주문번호', '반는분연락처', '받는분기타연락처', '받는분주소', '배송메시지']]
